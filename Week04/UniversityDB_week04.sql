...
-- Exercise
-- 4.1.1
SELECT StudID, StudName, DeptName, Building FROM Student NATURAL JOIN Department ORDER BY StudID;
-- 4.1.2
SELECT InstID , InstName , COUNT(SectionID) AS SectionsTought FROM Instructor NATURAL LEFT OUTER JOIN Teaches GROUP BY InstID , InstName;
-- LEFT OUTER is needed, otherwise instructors who do not teach would not appear in the result (with a 0).
-- 4.1.3
SELECT InstID, InstName, (SELECT COUNT(*) FROM Teaches WHERE Teaches.InstID = Instructor.InstID) AS SectionsTought FROM Instructor;
-- 4.1.4
CREATE VIEW SeniorStudents AS SELECT StudID, StudName FROM Student WHERE TotCredits > 100;
SELECT * FROM SeniorStudents;
-- 4.1.5
DROP VIEW IF EXISTS Creditview;
CREATE VIEW Creditview(StudyYear, SumCredits) AS (SELECT StudyYear, SUM(Credits) FROM Takes NATURAL JOIN Course GROUP BY StudyYear);
SELECT * FROM Creditview;
-- 4.2.1
SELECT DeptName, COUNT(InstID) AS Total FROM Department NATURAL LEFT OUTER JOIN Instructor GROUP BY DeptName;
-- 4.2.2
