...
-- Exercise
-- 3.1.1
SELECT CourseID , SectionID , Count(StudID) FROM Takes WHERE Semester = 'Fall' AND StudyYear = 2009 GROUP BY CourseID , SectionID;
-- 3.1.2
SELECT Title, SUM(Credits) FROM Instructor NATURAL JOIN Teaches NATURAL JOIN Course WHERE InstName = 'Brandt' GROUP BY Title;
-- 3.1.3
SET SQL_SAFE_UPDATES = 0;
DELETE FROM Course WHERE CourseID NOT IN (Select CourseID FROM Section);
SET SQL_SAFE_UPDATES = 1;
-- 3.2.1
SELECT MAX(Salary) FROM Instructor;
-- 3.2.2
SELECT InstID, InstName FROM Instructor WHERE Salary >= (SELECT MAX(Salary) FROM Instructor);
-- 3.2.3
SELECT * FROM Takes;
DELETE FROM Takes WHERE CourseID IN ('BIO-101', 'BIO-301');
SELECT * FROM Takes;
-- 3.2.4
SELECT studID FROM Student WHERE studID NOT IN (SELECT studID FROM Takes);
-- 3.2.5
SELECT DeptName FROM Department WHERE Budget >= ALL (SELECT Budget FROM Department);
-- 3.2.6
SELECT studName FROM Student WHERE studName = SOME (SELECT InstName FROM Instructor);
-- or
SELECT studName FROM Student WHERE studName IN (SELECT InstName FROM Instructor);
-- or
SELECT DISTINCT StudName FROM Student, Instructor WHERE StudName = InstName;
-- 3.2.7
...
INSERT GradePoints VALUES ('C-',1.7);
INSERT GradePoints VALUES ('F',0.0);
SELECT * FROM GradePoints;
-- 3.2.8
SELECT StudID, SUM(Credits*Points) AS 'Total Grade-Points' FROM (Takes NATURAL JOIN Course) NATURAL JOIN GradePoints GROUP BY StudID;
-- 3.2.9
SELECT StudID, SUM(Credits*Points)/SUM(Credits) AS 'GPA' FROM (Takes NATURAL JOIN Course) NATURAL JOIN GradePoints GROUP BY StudID ORDER BY GPA DESC;
-- 3.2.10
(SELECT StudID, NULL AS 'Total Grade-Points' FROM Student WHERE StudID NOT IN (SELECT StudID FROM Takes)) 
UNION (SELECT StudID, SUM(Credits*Points) AS 'Total Grade-Points' FROM (Takes NATURAL JOIN Course) NATURAL JOIN GradePoints GROUP BY StudID);

(SELECT StudID, NULL AS 'GPA' FROM Student WHERE StudID NOT IN (SELECT StudID FROM Takes)) 
UNION (SELECT StudID, SUM(Credits*Points)/SUM(Credits) AS 'GPA' FROM (Takes NATURAL JOIN Course) NATURAL JOIN GradePoints GROUP BY StudID) ORDER BY GPA DESC;
-- 3.2.11
...
SELECT StudName, MAX(Score) FROM Testscores GROUP BY StudName HAVING MAX(SCore)>49;
SELECT DISTINCT StudName From Testscores WHERE Score IS NULL; 